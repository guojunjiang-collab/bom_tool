  ...
  render: function(c) {
    ...
    function render() {
      c.innerHTML = '<div class="page-header"><h2>⚙️ 系统设置</h2></div>' +
        '<div class="card"><div class="card-header">字典管理</div><div class="card-body">' +
      ...
      // ===== 自定义字段管理卡片 =====
      '<div class="card" style="margin-top:16px"><div class="card-header">自定义字段管理</div><div class="card-body" id="cf-card-body">' +
  ...
  _clearAll: function() {
    UI.confirm('⚠️ 此操作将清除所有业务数据（零件、部件、BOM、附件、日志、自定义字段值），<strong>保留用户账号和系统设置</strong>（字典、自定义字段定义）。<br><br>本地与服务器的数据都将被清除，确定继续吗？', async function() {
      try {
        ...
        var keepKeys = ['bom_api_token', 'bom_current_user', 'bom_dict_materials', 'bom_custom_field_defs'];
        var toRemove = [];
